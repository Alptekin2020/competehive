...
export default function ProductsPage() {
  ...
  const mapCompareResult = (c: CompareResult): CompetitorItem => ({
  ...
  const getHighestPrice = (product: ProductItem) => {
    const prices = [
      product.current_price ? Number(product.current_price) : null,
      ...(product.competitors || []).map((c) => (c.current_price ? Number(c.current_price) : null)),
    ].filter(Boolean) as number[];
      ...
      {products.length > 0 ? (
        <div className="space-y-4">
          {products.map((product) => {
            ...
            const mpInfo = getMarketplaceInfo(product.marketplace);
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span
                        ...
                        style={{
                          backgroundColor: `${mpInfo.color}20`,
                          color: mpInfo.color,
                        }}
                      >
                        {mpInfo.name}
                  ...
                  <div className="text-right flex-shrink-0">
                    <div
                      className={`font-semibold ${isCheapest ? "text-green-400" : "text-white"}`}
                    >
                    ...
                    {product.competitors && product.competitors.length > 0 ? (
                      <div className="space-y-2">
                        {product.competitors
                          .sort(
                            (a, b) =>
                              (Number(a.current_price) || 999999) -
                              (Number(b.current_price) || 999999),
                          )
                          .map((comp, idx) => {
                            const compPrice = comp.current_price
                              ? Number(comp.current_price)
                              : null;
                            ...
                            return (
                              ...
                              >
                                <span
                                  className="text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0"
                                  style={{
                                    backgroundColor: `${comp.retailerColor || compMp.color}20`,
                                    color: comp.retailerColor || compMp.color,
                                  }}
                                >
                                  {comp.retailerName || compMp.name}
                                </span>
                                <a
                                  ...
                                  setProducts((prev) =>
                                    prev.map((p) =>
                                      p.id === product.id
                                        ? {
                                            ...p,
                                            competitors: compareData.competitors.map(
                                              (c: CompareResult) => mapCompareResult(c),
                                            ),
                                          }
                                        : p,
                                    ),
                      ...
                      <div className="mt-4 p-3 bg-dark-900 rounded-xl border border-dark-800">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="text-white font-medium">
                            {lowest.toLocaleString("tr-TR")} TL &mdash;{" "}
                            {highest.toLocaleString("tr-TR")} TL
                          </span>
        ...
        <div className="bg-dark-900 border border-dark-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-dark-500 text-sm mb-6">
            Bir marketplace linkini yapistirin &mdash; diger sitelerdeki fiyatlar otomatik
            bulunacak.
          </p>
          <button
            onClick={() => setShowModal(true)}
          ...
          <div className="bg-dark-900 border border-dark-800 rounded-2xl p-6 w-full max-w-lg relative z-10">
            ...
            <form onSubmit={handleAdd}>
              ...
              <p className="text-dark-600 text-xs mb-6">
                Herhangi bir e-ticaret sitesinin urun linkini yapistirin. Diger
                marketplace&apos;lerdeki fiyatlar otomatik bulunacak.
              </p>
