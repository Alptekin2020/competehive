...
async function main() {
  ...
  try {
    ...
    await client.query(
      `ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "telegram_connected_at" TIMESTAMP(3) DEFAULT NULL`,
    );
    ...
    await client.query(
      `ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "telegram_link_token_expires_at" TIMESTAMP(3) DEFAULT NULL`,
    );
